...
public class AtendimentoController implements ICrudController<Atendimento> {
    ...
    @GetMapping("/busca/{termoBusca}/{termoProfissional}")
    public ResponseEntity<List<String>> getByAll(@PathVariable("termoBusca") Date termoBusca, @PathVariable("termoProfissional") Long termoProfissional) {
        List<String> registros = servico.getByAllAtendimentos(termoBusca, termoProfissional);
        return new ResponseEntity<>(registros, HttpStatus.OK);
    }
